    ...
    <AdminLayout>
      <div className="max-w-2xl">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
          <div>
            <h1 className="text-xl font-black text-gray-900">Brands</h1>
            <p className="text-gray-500 text-sm">Brand ecosystem displayed by category</p>
          </div>
          <div className="flex items-center gap-3">
            <a href="/services/dock-equipment" target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gold text-sm flex items-center gap-1 min-h-[44px]">
        ...
        <div className="flex flex-col gap-6">
          {data.categories.map((cat, catIdx) => (
            <div key={catIdx} className="bg-gray-50 border border-gray-200 rounded-lg p-5">
              <div className="flex items-center justify-between mb-4">
                <input type="text" value={cat.name} onChange={(e) => updateCategoryName(catIdx, e.target.value)}
                  className="bg-transparent border-b border-gray-200 focus:border-gold text-gray-900 font-bold text-sm focus:outline-none pb-1 w-full max-w-xs" />
                <button onClick={() => removeCategory(catIdx)} className="text-gray-500 hover:text-red-600 transition-colors p-1 ml-3 min-h-[44px] flex items-center">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
              <div className="flex flex-wrap gap-2 mb-3">
                {cat.brands.map((brand, brandIdx) => (
                  <div key={brandIdx} className="flex items-center gap-1 bg-white border border-gray-200 rounded px-2 py-1">
                    <input type="text" value={brand} onChange={(e) => updateBrand(catIdx, brandIdx, e.target.value)}
                      className="bg-transparent text-gray-900 text-xs w-24 focus:outline-none" />
                    <button onClick={() => removeBrand(catIdx, brandIdx)} className="text-gray-500 hover:text-red-600 transition-colors" aria-label="Remove brand">
