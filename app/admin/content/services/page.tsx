      ...
      <div className="max-w-3xl">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">Edit each service listing</p>
          </div>
          <div className="flex items-center gap-3">
            <a href="/services" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-gold text-sm flex items-center gap-1 min-h-[44px]">
        ...
        <div className="flex flex-col gap-3">
          {items.map((service, i) => (
            <div key={i} className="bg-dark-secondary border border-dark-tertiary rounded-lg overflow-hidden">
              <button
                className="w-full flex items-center justify-between px-5 py-4 hover:bg-dark-tertiary/50 transition-colors min-h-[44px]"
                onClick={() => setExpanded(expanded === i ? null : i)}
              >
                <span className="text-white font-bold text-sm">{service.name || `Service #${i + 1}`}</span>
                {expanded === i ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
              </button>

              {expanded === i && (
                <div className="px-5 pb-5 flex flex-col gap-4 border-t border-dark-tertiary pt-4">
                  ...
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">Short Description (card)</label>
                  ...
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">Full Description (page)</label>
                    <textarea value={service.description} onChange={(e) => updateField(i, 'description', e.target.value)} rows={3}
                      className="w-full bg-dark-bg border border-dark-tertiary focus:border-gold rounded px-3 py-2 text-white text-sm focus:outline-none resize-y" />
                  </div>

                  <div>
                    <label className="block text-xs text-gray-400 mb-2">Service Bullets</label>
                    {service.bullets.map((b, bi) => (
                      <div key={bi} className="flex items-center gap-2 mb-2">
                        <input type="text" value={b} onChange={(e) => updateBullet(i, bi, e.target.value)}
                          className="flex-1 bg-dark-bg border border-dark-tertiary focus:border-gold rounded px-3 py-2 text-white text-sm focus:outline-none min-h-[44px]" />
                        <button onClick={() => removeBullet(i, bi)} className="text-gray-500 hover:text-red-400 transition-colors p-1 min-h-[44px] flex items-center">
